...
```{r M1_means_table}
kable(allsim.means.m1, format="html", row.names=FALSE, digits=2) %>% kable_styling("condensed", full_width = FALSE, position="left", font_size=12) %>% scroll_box(width="600px", height = "400px") 
```
...
Let's plot the dependence of the median time against each parameter:
...
```{r M1_heatmap_fc, fig.width=5, fig.height=3.5}
s <- subset(allsim.means.m1, detachment == 0.5)
s$conversion <- as.factor(s$conversion)
s$formation <- as.factor(s$formation)
ggplot(s) +
  geom_tile(aes(x=formation, y=conversion, fill=median.time)) +
  labs(x="Formation rate", y="Conversion rate", fill="Median time (min)") +
  scale_fill_viridis()
```
...
P <- lapply(c(0.125, 0.5, 2), function(d) {
  ...
  plotDistributionGrid(allsims, grid.par, fixed.par)
})
...
kable(allsim.means.m2, format="html", row.names=FALSE, digits=2) %>% kable_styling("condensed", full_width = FALSE, position="left", font_size=12) %>% scroll_box(width="600px", height = "400px") 
```

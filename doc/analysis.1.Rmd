...
saveRDS(kinconv, file=paste0(projectDir, "kinetube/convrate.rds"))
```

```{r kinetube_converstion_time}
kinconv <- readRDS(paste0(projectDir, "kinetube/convrate.rds"))
```
...
```{r shuyus_data}
sh <- read.table("../analysis_V5.txt", sep="\t", header=TRUE)
names(sh) <- c("cell", "time.s", "time.m")
sh.ci <- ciMedian(sh$time.m, nboot=10000)
```
...
The confidence limits on the median were found using a bootstrap: the original data were resampled with replacement 10,000 times, median was recorded for each bootstrap and the 2.5% and 97.5% percentiles were found across all 10,000 simulated medians.

...
It contains several statistics for each kinetochore, over many simulations. Among them is the total sliding time. This is not exactly what we want, as this is the accumulated time, so includes KT that were detached and re-attached. But the simulation also recorded the total drop-off time, that is the time the KT spend while being knocked-off before being re-attached. We select only data where total dropoff time is zero, which corresponds to KTs that have not been knocked-off. The mean sliding time from this file is `r round(mean.sliding.time, 2)` minutes, which gives the conversion rate of `r round(1/mean.sliding.time, 2)` per min.
...
```{r all_sims_mean}
allsim.means <- allsims %>% group_by(model, formation, conversion, detachment, knockoff, replacement) %>% summarise(mean.time = mean(time), se.time = sd(time) / sqrt(n()), median.time=median(time), median.detached=median(detached), median.detached.fraction=median(detached/time), n=n()) %>% as.data.frame
allsim.means <- allsim.means[order(allsim.means$mean.time), c(1:(ncol(allsim.means)-1))]
allsim.means.m1 <- allsim.means[allsim.means$model == "M1", c(M1.parnames, "mean.time", "se.time", "median.time", "median.detached.fraction")]
allsim.means.m2 <- allsim.means[allsim.means$model == "M2", c(M2.parnames, "mean.time", "se.time", "median.time", "median.detached.fraction")]
```
...
Simply, we start from the default values and vary each parameter by factor two, up and down (except for knockoff). For each set we run `r N(as.integer(nrow(allsims) / nrow(allsim.means)))` simulations.
...
```{r M1_means_table}
kable(allsim.means.m1, format="html", row.names=FALSE, digits=2) %>% kable_styling("condensed", full_width = FALSE, position="left", font_size=12) %>% scroll_box(width="700px", height = "400px") 
```

```{r M1_all_sims_linear_model, eval=FALSE}
fit1 <- lm(median.time ~ formation + conversion + detachment, data=allsim.means.m1)
summary(fit1)
```
...
plotParameterMedianTime(allsim.means.m1, M1.parnames)
```
...
g1 <- plotTwoPar(allsim.means.m1, "formation", "conversion", "detachment", 0.25, ylim=c(1, 400))
g2 <- plotTwoPar(allsim.means.m1, "formation", "conversion", "detachment", 2, ylim=c(1, 400))
...
g1 <- plotTwoPar(allsim.means.m1, "conversion", "formation", "detachment", 0.25, ylim=c(1, 400))
g2 <- plotTwoPar(allsim.means.m1, "conversion", "formation", "detachment", 2, ylim=c(1, 400))
grid.arrange(g1, g2, ncol=2)
```
...
We need high formation and conversion rates, and, perhaps, a low detachment rate.

### Heatmaps

Here is a heatmap of formation versus conversion, for fixed detachment rates of 0.125 and 1:

```{r M1_heatmaps, fig.width=10, fig.height=3.5}
g1 <- plotTwoHeat(allsim.means.m1, "formation", "conversion", "detachment", 0.125)
g2 <- plotTwoHeat(allsim.means.m1, "formation", "conversion", "detachment", 1)
grid.arrange(g1, g2, ncol=2)
```
...
P <- lapply(c(0.125, 0.5, 2), function(d) {
  ...
  plotDistributionGrid(allsims, "M1", grid.par, fixed.par)
})
...
g1 <- plotTwoPar(allsim.means.m1, "detachment", "formation", "conversion", 2, val="median.detached.fraction", val.name="Median detached fraction", log.scale = FALSE)
g2 <- plotTwoPar(allsim.means.m1, "formation", "conversion", "detachment", 2, val="median.detached.fraction", val.name="Median detached fraction", log.scale = FALSE)
grid.arrange(g1, g2, ncol=2)
```
...
kable(allsim.means.m2, format="html", row.names=FALSE, digits=2) %>% kable_styling("condensed", full_width = FALSE, position="left", font_size=12) %>% scroll_box(width="700px", height = "400px") 
```
...
plotParameterMedianTime(allsim.means.m2, M2.parnames)
```
...
g1 <- plotTwoPar(allsim.means.m2, "replacement", "conversion", "knockoff", 1e16, ylim=c(0.7, 300))
g2 <- plotTwoPar(allsim.means.m2, "replacement", "conversion", "knockoff", 1, ylim=c(0.7, 300))
grid.arrange(g1, g2, ncol=2)
```
...
```{r M2_heatmaps, fig.width=10, fig.height=3.5}
g1 <- plotTwoHeat(allsim.means.m2, "replacement", "conversion", "knockoff", 1e16)
g2 <- plotTwoHeat(allsim.means.m2, "replacement", "conversion", "knockoff", 1)
grid.arrange(g1, g2, ncol=2)
```

### Distribution plots

```{r M2_plot_distribution_grid, fig.width=8, fig.height=12}
grid.par <- list(replacement=c(0.25, 1, 4), conversion=c(0.25, 1, 4))
P <- lapply(c(1e16, 1), function(d) {
  fixed.par <- list(knockoff=d)
  plotDistributionGrid(allsims, "M2", grid.par, fixed.par, xlim=c(0.01, 500))
})
grid.arrange(grobs=P, ncol=1) 
```
...
and examples as above, but with $R_\mathrm{ko}=1$:
...
The model described in this document simulates error correction for one chromosome (sister chromatids). In yeast, we have 16 chromosomes. What is the total time required to correct all chromosomes and establish proper bi-orientation? If error correction for all of them is done in parallel, then the total time is the maximum time from one simulation. We can model this by randomly sampling 16 timelines from our simulation results and finding the maximum time. NOTE: this is a very conservative approach assuming that *all* chromosomes start in an erroneous configuration.

```{r max_16_chromosomes_function}
maxChromosomes <- function(sim, model, pars, n.chromosomes=16, n.boot=1000, n.cores=7) {
  sim <- sim[sim$model == model, ]
  for(p in names(pars)) {
    val <- pars[[p]]
    sim <- sim[sim[[p]] == val,]
...
m1.max <- maxChromosomes(sim, model, pars)
breaks <- seq(0, 20, 0.5)
...
We select data with the following parameters:

- M1: formation=2, conversion=2, detachment=0.5
- M2: replacement=2, conversion=2, knockoff=1e16

```{r M1_max_chr, fig.width=6, fig.height=3}
g1 <- plotMaxChromosomes(allsims, "M1", list(formation=2, conversion=2, detachment=0.5))
g2 <- plotMaxChromosomes(allsims, "M2", list(replacement=2, conversion=2, knockoff=1e16))
grid.arrange(g1, g2, ncol=2)
```

...
Here is the distribution of individual conversion times:
...
```{r conversion_time_distribution, fig.width=4, fig.height=3}
...
Here are distribuions of simulation time for both models (up and down) and parameter sets (fill colours).

```{r defpar_plot, fig.width=6, fig.height=3}
defparRidgePlot(deftime)
```
...
Hence, we can simplify distribution plots. We fix $R_\mathrm{conv} = 1.5$ and $R_\mathrm{det} = 1~\mathrm{min}^{-1}$ and show the full range of formation rate.

```{r detached_distribution_time, fig.width=5, fig.height=4}
...
ggplot(m, aes(x=detached.time, y=form)) +
  geom_density_ridges() +
  scale_x_log10(breaks=brks, labels=brks, limits=c(0.001, 50)) +
...
Next, we find the diffusion range, defined as $\sigma = \sqrt{6 D t}$, where $D = 0.1~\mu \mathrm{m}^2\mathrm{min}^{-1}$ is the diffusion coefficient and $t$ is the detachment time. We fix $R_\mathrm{conv} = 1.5$ and $R_\mathrm{det} = 1~\mathrm{min}^{-1}$ and show the full range of formation rate.
...
```{r detached_distribution_distance, fig.width=5, fig.height=4}
sig1 <- median(m[m$formation == 1, "sigma"])
ggplot(m, aes(x=sigma, y=form)) +
  geom_density_ridges() +
  scale_x_log10(breaks=brks, labels=brks, limits=c(0.02, 8)) +
  theme_ridges(center_axis_labels = TRUE) +
  #scale_fill_viridis(discrete=TRUE) +
  labs(x=expression(sigma~(mu*m)), y=expression(R[form]~(min^-1)))
```

...
figdata.dir <- paste0(projectDir, "figdata/")
if(!dir.exists(figdata.dir)) dir.create(figdata.dir)
```
...
The lateral takeover switches the side of the MT. E.g., it starts with an `endon-L` configuration and ends up with `endon-R` configuration.

#### Same side: failed takeover

When the lateral takeover is attempted from the same side, observations suggest that it fails to knock-off the existing end-on attachment. Instead, a "synlink" attachment is formed (at replacement rate, $R_\mathrm{rep}$):
...
This works like a normal lateral attachment and is converted into endon at conversion rate $R_\mathrm{conv}$, just like above, except that at the end of the process a "biend" (bi-end-on) attachment is formed.
...
This configuration is unstable and the new MT (the one that was trying to take over) is the detached at the knock-off rate $R_\mathrm{ko}$:
...
The failed lateral takeover does not change the side of the MT.

#### Both takeovers have the same duration

Note: both opposite-side and same-side lateral takeover take the same amount of time. This is because the opposite-side takeover is a sequence of events with rates $R_\mathrm{rep}$, $R_\mathrm{ko}$ and $R_\mathrm{conv}$, while the same-side takeover is a sequence of events with rates $R_\mathrm{rep}$, $R_\mathrm{conv}$ and $R_\mathrm{ko}$. The only difference is the switched order of the last two events, but the total time (on average) will be the same.
...
```{r kinetube_result_original, eval=FALSE}
...
```{r kinetube_mean_sliding_time}
mean.sliding.time <- 0.6655942  # copied from the above result
```
...
```{r kinetube_conversion_time_raw, eval=FALSE}
...
saveRDS(kinconv, file="../kinetube/convrate.rds")
```

```{r kinetube_converstion_time}
kinconv <- readRDS("../kinetube/convrate.rds")
```

```{r plot_conversion_time_function}
plotConversionTimeDistribution <- function(kinconv, log.scale=TRUE) {
...
```{r conversion_time_distribution, fig.width=8, fig.height=3, message=FALSE, warning=FALSE}
mean.conv <- mean(kinconv$time)
conv.rate <- 1 / mean.conv
g1 <- plotConversionTimeDistribution(kinconv, log.scale=FALSE)
g2 <- plotConversionTimeDistribution(kinconv, log.scale=TRUE)
g <- grid.arrange(g1, g2, ncol=2)

link <- pdfFigure(g, "convrate.pdf")

fd <- data.frame(time = round(kinconv$time, 3))
dlink <- figData(fd, "kinconv.csv")
```

Link to the figure: `r link`.
...
```{r ci_median_function}
ciMedian <- function(x, nboot=1000, ci=0.95) {
  n <- length(x)
  P <- unlist(lapply(1:nboot, function(i) {
...
```{r shuyus_data}
sh8 <- read.table("../analysis_V8.txt", sep="\t", header=TRUE)
names(sh8) <- c("cell", "time.s", "time.m")
sh.med <- median(sh8$time.m)
sh.ci <- ciMedian(sh8$time.m, nboot=10000)
```

This plots shows Shuyu's data for bi-orientation time in `r nrow(sh8)` cells:

```{r shuyus_data_plot, fig.width=3.5, fig.height=2}
ggplot(sh8, aes(x=time.m)) +
  geom_dotplot(binwidth=0.3) +
  scale_y_continuous(name="", breaks=NULL) +
  theme_classic() +
  labs(x="Bi-orientation time (min)") +
  geom_vline(xintercept = sh.med, colour="lightblue3") +
  geom_vline(xintercept = sh.ci[1], colour="lightblue3", linetype="dotted") +
  geom_vline(xintercept = sh.ci[2], colour="lightblue3", linetype="dotted")
```

The median is `r round(sh.med, 2)` min (solid line) and the 95% confidence intervals are [`r round(sh.ci[1], 2)`, `r round(sh.ci[2], 2)`] min (dotted lines). The confidence limits on the median were found using a bootstrap: the original data were resampled with replacement 10,000 times, median was recorded for each bootstrap and the 2.5% and 97.5% percentiles were found across all 10,000 simulated medians.


Here we create a plot for the paper. Following our previous simulations and Vanya's paper we fix $R_\mathrm{form} = 2~\mathrm{min}^{-1}$ and $R_\mathrm{conv} = 2~\mathrm{min}^{-1}$. Shuyu's observations suggest median bi-orientation time of about 1 min. Hence, for these two fixed parameters we seek the detachment rate with the median run-time closest to 1 min.

```{r M1_fig_best_parameters_search, fig.width=3, fig.height=3}
mm1.sel <- subset(time.m1, formation==2 & conversion==2)
g <- plotBestTime(mm1.sel, "detachment", ylim=c(0.2, 200))
g

link <- pdfFigure(g, "M1_best_time.pdf")

fd <- setNames(mm1.sel[, c("detachment", "time")], c("Detachment_rate", "Time"))
fd$Time <- signif(fd$Time, 4)
dlink <- figData(fd, "m1_best_params.csv")
```
...
The best parameter is $R_\mathrm{det} = 1~\mathrm{min}^{-1}$. Here is the distribution of individual run times for this parameter. The orange solid line shows the median of the distribution, while the blue dotted lines indicate 95% confidence intervals on the median bi-orientation time from Shuyu's observations.


```{r M1_fig_runtime_distribution, fig.width=7, fig.height=3, message=FALSE, warning=FALSE}
m1.sel <- subset(time.m1, formation==2 & conversion==2 & detachment==1)
#g <- plotDistributionPar(m1.sel, xlim=c(0.08, 30), show=5, bins=50, lines=sh.ci)
g <- plotDistributionPar(m1.sel, xlim=c(0.01, 30), ymax.log=1.05, ymax.lin=0.45, show=5, bins=50, lines=sh.ci)

link <- pdfFigure(g, "M1_defpar.pdf")
```

Here is a link to the figure: `r link`. The median of this distribution is  `r sprintf("%4.2f", median(m1.sel$time))`.

### Examples

A few examples of simulation runs for a fast model 1 (that is $R_\mathrm{form}=2$, $R_\mathrm{conv}=2$ and $R_\mathrm{det}=1$):

```{r examples_function}
plotExamples <- function(model, pars, seed, root="example", n=5, max.time=15, time.limits=NULL) {
  set.seed(seed)
  P <- list()
  for(i in 1:n) {
    if(is.null(time.limits)) {
      sc <- simulate(model, pars)
    } else {
      repeat {
        sc <- simulate(model, pars)
        if(sc$time >= time.limits[1] && sc$time <= time.limits[2]) {
          break
        }
      }
    }
    
    g <- plotStateHistory(sc, max.time=max.time)
    print(g)
  
    file <- paste0(model, "_", root, "_", i, ".pdf")
    pfile <- paste0(pdf.dir, file)
    ggsave(pfile, plot=g, device="pdf")

    P[[i]] <- data.frame(link=paste0("[PDF", i, "](", public_html, "pdf/", file, ")"))
  }
  df <- do.call(rbind, P)
  myKable(df)
}
...
plotExamples("M1", parametersRates(formation = 2, conversion = 2, detachment = 1), seed=67, max.time=10.5)
```

### Examples with time limit

Here are examples with more typical run time between 2.6 and 3.4 min.

```{r M1_fast_simulations_examples_time, fig.width=10, fig.height=1.5}
plotExamples("M1", parametersRates(formation = 2, conversion = 2, detachment = 1), root="example_tl", seed=100, n=5, max.time=3.5, time.limits = c(2.6, 3.4)) 
```
...
The best parameter is $R_\mathrm{ko} = 10^{16}~\mathrm{min}^{-1}$. However, Hari's in vitro experiment show that it typically takes 5-6 seconds until kinetochores detach from end-on attachment after a lateral attachment is formed. Therefore, the above results is probably not too realistic and we select $R_\mathrm{ko} = 10~\mathrm{min}^{-1}$ instead. Here is the distribution of individual run times for this parameter. The orange solid line shows the median of the distribution, while the blue dotted lines indicate 95% confidence intervals on the median bi-orientation time from Shuyu's observations.

```{r M2_fig_runtime_distribution, fig.width=7, fig.height=3}
m2.sel <- subset(time.m2, replacement==2 & conversion==2 & knockoff==10)
#g <- plotDistributionPar(m2.sel, xlim=c(0.01, 20), show=5, bins=50, lines=sh.ci)
g <- plotDistributionPar(m2.sel, xlim=c(0.01, 30), ymax.log=1.05, ymax.lin=0.45, show=5, bins=50, lines=sh.ci) 

link <- pdfFigure(g, "M2_defpar.pdf")
```

Here is a link to the figure: `r link`. The median of this distribution is  `r sprintf("%4.2f", median(m2.sel$time))`.

### Best parameters distribution plot: comparison

```{r M1_M2_fig_runtime_distribution, fig.width=7, fig.height=3}
m1.sel <- subset(time.m1, formation==2 & conversion==2 & detachment==1)
m2.sel <- subset(time.m2, replacement==2 & conversion==2 & knockoff==10)
g <- plotDistributionPar2(m1.sel, m2.sel, xlim=c(0.01, 30), alpha=0.6, show=5, bins=50, lines=sh.ci, line.colour="black") 

link <- pdfFigure(g, "M1_M2_defpar.pdf")

fd <- data.frame(
  Model = c(rep("M1", nrow(m1.sel)), rep("M2", nrow(m2.sel))),
  Time = signif(c(m1.sel$time, m2.sel$time), 4)
)
dlink <- figData(fd, "m1_m2_best_comparison.csv")

fsh <- data.frame(Cell=sh8$cell, Time=round(sh8$time.m, 2))
slink <- figData(fsh, "cell_data.csv")
```

Here is a link to the figure: `r link`.

Here is a link to `r dlink` and additional `r slink` for Shuyu's data (required to calculated median confidence interval).

### Comparison to Shuyu's data

```{r ci_prop}
propCI <- function(s, n, alpha=0.95) {
  Z <- -qnorm((1 - alpha)/2)
  pp <- (s + Z^2/2) / (n + Z^2)
...
```{r bin_shuyus_data}
range <- 0:8
sh8.bin <- table(cut(sh8$time.m, range))
sh8.bin <- data.frame(
  mid.time = range[-1] - 0.5,
  count = as.integer(sh8.bin)
)
n <- sum(sh8.bin$count)
sh8.prop <- propCI(sh8.bin$count, n, alpha = 2*(pnorm(1) - 0.5))
sh8.bin <- cbind(sh8.bin, sh8.prop)
sh8.bin[sh8.bin$lo < 0, "lo"] <- 0
```

```{r bin_models}
m1.bin <- as.numeric(table(cut(m1.sel$time, range)))
m2.bin <- as.numeric(table(cut(m2.sel$time, range)))
m.bin <- data.frame(
  x = range[-1],
  m1 = m1.bin / nrow(m1.sel),
  m2 = m2.bin / nrow(m2.sel)
)
sh8.bin <- cbind(sh8.bin, m.bin) 
```

Here is a combined histogram of Shuyu's data (grey bars), and the models (dark orange and blue lines show M1 and M2, respectively), aggregated at a bin size of 1 min. Error bars are 68% confidence intervals of a proportion, representing asymmetric standard error. When comparing model with data standard errors are appropriate as a measure of the distance between data and the model. Then, the distance between a data point and the model line expressed in error bars is a Z-score.

```{r shuyu_comparison_figure, fig.width=4, fig.height=3}
g <- ggplot(sh8.bin) +
  theme_classic() +
  geom_col(aes(x=mid.time, y=p), fill="grey80", colour="grey50", width=1) +
  geom_step(aes(x=mid.time-0.5, y=m1), stat="identity", colour="darkgoldenrod4", size=0.8) +
  geom_step(aes(x=mid.time-0.5, y=m2), stat="identity", colour="dodgerblue4", size=0.8) +
  #geom_point(aes(x=mid.time, y=p)) +
  geom_errorbar(aes(x=mid.time, ymin=lo, ymax=up), width=0.2, colour="grey50") +
  scale_x_continuous(breaks=0:10, limits=c(0,7), expand=c(0,0)) +
  scale_y_continuous(expand=c(0,0)) +
  labs(x="Bi-orientation time (min)", y="Density")
g
link <- pdfFigure(g, "M1_M2_Shuyu.pdf", width=4, height=3)

fd <- sh8.bin[,-6]
fd <- setNames(fd, c("Mid_time", "Count", "Proportion", "Prop.lower", "Prop.upper", "Model_1", "Model_2"))
dlink <- figData(fd, "model_comparison_binned.csv")
```

Here is a link to the figure: `r link`.

Here is a link to `r dlink`.

We can use the Kolmogorov-Smirnov test to compare both our models with the observed data. Here are the results:

```{r M1_M2_Shuyu_KS}
ks1 <- ks.test(m1.sel$time, sh8$time.m)
ks2 <- ks.test(m2.sel$time, sh8$time.m)
df <- data.frame(
  model = c("M1", "M2"),
  p.value = sprintf("%.2g", c(ks1$p.value, ks2$p.value))
)
myKable(df, col.names = c("Model", "p-value"))
```

Model 1 is significantly different from Shuyu's data, while the null hypothesis about model 2 (data and model come from the same distribution) cannot be rejected.

Note: because of the presence of ties (that is, in Shuyu's data there are multiple measurements with the same time), the p-values are approximate, though because we have large samples, approximations should be good.
...
plotExamples("M2", parametersRates(replacement = 2, conversion = 2, knockoff = 1e16), seed=67, max.time=4)
```

### Examples with time limit

Here are examples with more typical run time between 1.5 and 2.1 min.

```{r M2_fast_simulations_examples_time, fig.width=10, fig.height=1.5}
plotExamples("M2", parametersRates(replacement = 2, conversion = 2, knockoff = 10), root="example_tl", seed=100, n=5, max.time=3.5, time.limits = c(1.5, 2.1)) 
```
...
plotExamples("M2", parametersRates(replacement = 2, conversion = 2, knockoff = 1), root="example_delay", seed=12, max.time=16) 
```
...
```{r detached_stats}
mdet.m1 <- as.data.frame(det.m1) %>% group_by(formation, conversion, detachment) %>% summarise(median.detached.time=median(detached.time), n=n()) %>% arrange(median.detached.time) %>% as.data.frame
```
...
maxChromosomes <- function(sim, pars, n.chromosomes=16, n.boot=10000, n.cores=7) {
  for(p in names(pars)) {
    val <- pars[[p]]
    sim <- sim[sim[[p]] == val,]
  }
  d <- mclapply(1:n.boot, function(i) {
    max(sample(sim$time, n.chromosomes))
  }, mc.cores=n.cores)
  unlist(d)
}

plotMaxChromosomes <- function(mxc, model) {
  brks <- c(1, 2, 5, 10, 20, 50, 100)
  #breaks <- seq(0, 25, 0.5)
  ggplot(data.frame(x=mxc)) +
    theme_classic() +
    geom_histogram(aes(x=x, y=..density..), bins=40) +
    labs(title=paste("Model", model), x="Max time (min)", y="Density") +
    scale_x_log10(breaks=brks, labels=brks, limits=c(1,50))
}
```

We select data with the following (default) parameters (units are 1 / minute).

- M1: $R_\mathrm{form} = 2$, $R_\mathrm{conv} = 2$, $R_\mathrm{det} = 1$
- M2: $R_\mathrm{repl} = 2$, $R_\mathrm{conv} = 2$, $R_\mathrm{ko} = 10$

These plots show the distribution of the maximum time across 16 randomly selected samples:

```{r max_chromosomes}
mxc1 <- maxChromosomes(time.m1, list(formation=2, conversion=2, detachment=1))
mxc2 <- maxChromosomes(time.m2, list(replacement=2, conversion=2, knockoff=10))
```

```{r max_chromosomes_plot, fig.width=6, fig.height=3}
g1 <- plotMaxChromosomes(mxc1, "M1")
g2 <- plotMaxChromosomes(mxc2, "M2")
grid.arrange(g1, g2, ncol=2)
```

```{r max_chr_perc_func}
maxChrLimits <- function(mxc, lims = c(10, 20, 30, 40, 50)) {
  frc <- unlist(lapply(lims, function(lim) {
    length(which(mxc > lim)) / length(mxc)
  }))
  data.frame(
    limit = lims,
    percentage = 100*frc
...
Percentage of maximum time greater than the limit for model 1:

```{r m1_max_chr_perc}
mxp1 <- maxChrLimits(mxc1)
myKable(mxp1, digits=3) 
```

and for model 2:

```{r m2_max_chr_perc}
mxp2 <- maxChrLimits(mxc2)
myKable(mxp2, digits=3) 
```

### Run-time fractions for 1 chromosome

And here, for comparison, we have the same tables, but calculated for just one chromosome.

Model 1:

```{r m1_1_chr_perc}
t1 <- subset(time.m1, formation==2 & conversion==2 & detachment==1)
mxp1 <- maxChrLimits(t1$time)
myKable(mxp1, digits=3) 
```
...
```{r m2_1_chr_perc}
t2 <- subset(time.m2, replacement==2 & conversion==2 & knockoff==10)
mxp2 <- maxChrLimits(t2$time)
myKable(mxp2, digits=3) 
```

...
n <- sum(sh8.bin$count)
sh8.prop <- propCI(sh8.bin$count, n, alpha = 0.68)
...
Here is a combined histogram of Shuyu's data, orange and blue lines show M1 and M2. Wrror bars are 68% confidence intervals of a proportion, which is the nearest representation of standard errors, but with asymmetry. When comparing model and data standard errors are appropriate as a measure of the distance between data and model. The distance between a data point and the model expressed in standard errors is a Z-score.

```{r shuyu_comparison_figure, fig.width=4, fig.height=3}
g <- ggplot(sh8.bin) +
  theme_classic() +
  geom_step(aes(x=mid.time-0.5, y=m1), stat="identity", colour="orange") +
  geom_step(aes(x=mid.time-0.5, y=m2), stat="identity", colour="lightblue2") +
  geom_point(aes(x=mid.time, y=p)) +
  geom_errorbar(aes(x=mid.time, ymin=lo, ymax=up), width=0.2) +
  scale_x_continuous(breaks=0:10, limits=c(0,7), expand=c(0,0)) +
  scale_y_continuous(expand=c(0,0)) +
  labs(x="Time (min)", y="Density")
g
link <- pdfFigure(g, "M1_M2_Shuyu.pdf")
```

...
A lateral attachment is created at a rate $R_\mathrm{form}$.
...
Next, the lateral attachment is converted into the end-on attachment, at a rate $R_\mathrm{conv}$:
...
Finally, the microtubule is detached at a rate $R_\mathrm{det}$:
...
Then, a lateral attachment can be created at the same KT, at a rate $R_\mathrm{rep}$. This is the same process as "formation" in model 1, but we call it "replacement" here, for distinction. For a moment we have a "dual" attachment.
...
As before, the lateral attachment is converted into end-on at a rate $R_\mathrm{conv}$.

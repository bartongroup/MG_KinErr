...
Here are distribuions of simulation time for both models (up and down) and parameter sets (fill colours). We show the same data in linear of logarithmic scale.
...
Hence, we can simplify distribution plots. We fix $R_\mathrm{conv} = 1.5$ and $R_\mathrm{det} = 1~\mathrm{min}^{-1}$ and show the full range of formation rate (the same distribution in linear of logarithmic scale).
...
plotDistributionRidges <- function(m, x, y, xlab, ylab, log.scale=TRUE, limits=c(0.001, 50), smooth=TRUE, binwidth=0.1) {
  ...
  g <- ggplot(m, aes_string(x=x, y=y)) +
    ...
    labs(x="Detached time (min)", y=expression(R[form]~(min^-1)))
  if(smooth) {
...
m <- det.m1 %>%
  filter(conversion == 1.5 & detachment == 1) %>%
  mutate(form = as.factor(formation), sigma = sqrt(6 * 0.1 * detached.time))
g2 <- plotDistributionRidges(m, "detached.time", "form", "Detached time (min)", expression(R[form]~(min^-1)))
g1 <- plotDistributionRidges(m, "detached.time", "form", "Detached time (min)", expression(R[form]~(min^-1)), log.scale=FALSE, limits=c(0,20), smooth=FALSE, binwidth=0.1)
grid.arrange(g1, g2, ncol=2)
```
...
Next, we find the diffusion range, defined as $\sigma = \sqrt{6 D t}$, where $D = 0.1~\mu \mathrm{m}^2\mathrm{min}^{-1}$ is the diffusion coefficient and $t$ is the detachment time. We fix $R_\mathrm{conv} = 1.5$ and $R_\mathrm{det} = 1~\mathrm{min}^{-1}$ and show the full range of formation rate (the same distribution in linear of logarithmic scale).

```{r detached_distribution_distance_old, fig.width=8, fig.height=3, eval=FALSE}
...
sig1 <- median(m[m$formation == 1, "sigma"])
g2 <- plotDistributionRidges(m, "sigma", "form", expression(sigma~(mu*m)), expression(R[form]~(min^-1)))
g1 <- plotDistributionRidges(m, "sigma", "form", expression(sigma~(mu*m)), expression(R[form]~(min^-1)), log.scale=FALSE, limits=c(0,4), smooth=FALSE, binwidth=0.02)
grid.arrange(g1, g2, ncol=2)
```

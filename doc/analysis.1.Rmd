...
```{r examples_function}
plotExamples <- function(model, pars, seed, n=5, max.time=15, time.limits=NULL) {
  set.seed(seed)
  P <- list()
  for(i in 1:n) {
    ...
    file <- paste0(model, "_example_", i, ".pdf")
    pfile <- paste0(pdf.dir, file)
    ggsave(pfile, plot=g, device="pdf")
...
plotExamples("M1", parametersRates(formation = 2, conversion = 2, detachment = 1), seed=100, n=5, max.time=3.5, time.limits = c(2.6, 3.4)) 
```
...
plotExamples("M2", parametersRates(replacement = 2, conversion = 2, knockoff = 1e16), seed=100, n=5, max.time=3.5, time.limits = c(1.1, 1.7)) 
```
...
plotExamples("M2", parametersRates(replacement = 2, conversion = 2, knockoff = 1), seed=12, max.time=16) 
```

...
Here is a combined histogram of Shuyu's data (grey bars), and the models (dark orange and blue lines show M1 and M2, respectively), aggregated at a bin size of 1 min. Error bars are 68% confidence intervals of a proportion, representing asymmetric standar error. When comparing model with data standard errors are appropriate as a measure of the distance between data and the model. Then, the distance between a data point and the model line expressed in error bars is a Z-score.
...
Note: because of the presence of ties (that is, in Shuyu's data there are multiple measurements with the same time), the p-values are approximate, though because we have large samples, approximations w should be good.
